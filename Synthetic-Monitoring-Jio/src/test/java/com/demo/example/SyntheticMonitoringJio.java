...
import com.demo.SyntheticMonitoring;
...
public class SyntheticMonitoringJio {
    ...
    private static void processEntry(Map<String, Object> entry) throws InterruptedException {
        ...
        String pageSource = driver.getPageSource();  // Get page content (response body)
        String timestamp = new Date().toString();    // Get current timestamp
        
        StringBuilder emailBody = new StringBuilder();
        
        emailBody.append("Monitoring Report for URL: ").append(currentUrl).append("\n");
        emailBody.append("Timestamp: ").append(timestamp).append("\n");
        emailBody.append("Expected Response Time: ").append(expectedResponseTime).append(" ms\n");
        emailBody.append("Actual Response Time: ").append(responseTime).append(" ms\n");
        emailBody.append("Expected Response Code: ").append(expectedResponseCode).append("\n");
        emailBody.append("Actual Response Code: ").append(responseCode).append("\n");
        emailBody.append("Response Body Preview (first 500 characters): \n")
                 .append(pageSource.substring(0, Math.min(pageSource.length(), 500))).append("\n");
        ...
        if (responseTime > expectedResponseTime) {
            ...
            emailBody.append("Warning: The response time exceeded the expected time.\n");
        } else {
        ...
        if (responseCode == expectedResponseCode) {
            ...
            emailBody.append("Warning: The response code did not match the expected code.\n");
            emailBody.append("Expected Response Code: ").append(expectedResponseCode).append("\n");
            emailBody.append("Actual Response Code: ").append(responseCode).append("\n");
        }
        ...
        if (!isMatchFound) {
            ...
            emailBody.append("Warning: None of the expected content was found on the page.\n");
        } else {
        ...
        if (emailBody.length() > 0) {
            sendEmail("Alert Notification for URL Monitoring", emailBody.toString());
        }
